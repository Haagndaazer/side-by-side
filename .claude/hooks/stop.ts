      ...
      if (os_platform === 'darwin') {
        ...
        if (existsSync(soundFile)) {
          ...
          if (os_platform === 'win32') {
            ...
            // Linux/WSL - use Windows audio via WSL
            const windowsPath = soundFile.replace('/mnt/', '').replace('/', ':/').replace(/\//g, '\\');
            cmd = `powershell.exe -c "(New-Object Media.SoundPlayer '${windowsPath}').PlaySync()"`;
          }
          ...
          console.error('Please ensure on-agent-complete.wav exists in the .claude directory');
          afterNotification();
        }
      }
    }
